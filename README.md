...
Note that our method for property verification is fundamentally different from the approach used in [Lynx](https://github.com/josevalim/lynx). Although both are based on the idea of turning contracts and program properties into proof obligations,

 - Lynx translates Elixir programs into Lean by using a shallow embedding of functions (Elixir functions become Lean functions and their behaviour is defined by host language semantics). That is, this approach incorporates unverified, trusted code base (which brings programs and their specifications into the verification tool), and necessitates further (nontrivial) validation of whether the property proved formally is reflected in the program's actual behaviour.
 - In contrast, our approach uses deep embedding (program behaviour is defined explicitly) and defines the behaviour over the abstract syntax deeply embedded in the theorem prover. Thus, programs can be processed by the semantics without translation, but by a trivial mapping from the abstract format to the abstract syntax used in the theorem prover. This allows us to bring programs into the verification pipeline effortlessly.
